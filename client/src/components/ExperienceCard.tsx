...
export function ExperienceCard({ image, title, description, quote }: ExperienceCardProps) {
  return (
    <Card className="overflow-hidden hover-elevate transition-transform duration-300" data-testid={`card-experience-${title.toLowerCase().replace(/\s+/g, '-')}`}>
      ...
      <CardContent className="p-8 space-y-4">
        ...
        {quote && (
          <p className="italic text-sm text-muted-foreground border-l-2 border-primary/30 pl-4" data-testid={`text-experience-quote-${title.toLowerCase().replace(/\s+/g, '-')}`}>
            {quote}
          </p>
        )}
      </CardContent>
